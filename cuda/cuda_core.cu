...
__global__ void ProlongatePackedKernel(float4* out_data,
                                       ...
                                       int3 coarse_dim, float3 inverse_size_f,
                                       float3 inverse_size_c)
{
    int block_offset = gridDim.x * gridDim.y * blockIdx.z +
        gridDim.x * blockIdx.y + blockIdx.x;
    int index = block_offset * blockDim.x * blockDim.y * blockDim.z +
        blockDim.x * blockDim.y * threadIdx.z + blockDim.x * threadIdx.y +
        threadIdx.x;
                            ...
                            cudaArray* fine_array, int coarse_width)
{
    ...
    int tile_height = 8;
    int tile_width = 8;
    float3 inverse_size_f = make_float3(1.0f / fine_width, 1.0f / fine_width,
                                        1.0f / fine_width);
    float3 inverse_size_c = make_float3(1.0f / coarse_width, 1.0f / coarse_width,
                                        1.0f / coarse_width);
    ProlongatePackedKernel<<<grid, block>>>(dest_array, num_of_block_per_slice,
                                            tile_height, tile_width, coarse_dim,
                                            inverse_size_f, inverse_size_c);
