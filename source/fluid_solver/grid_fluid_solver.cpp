...
void GridFluidSolver::Impulse(GraphicsVolume* density, float splat_radius,
                              ...
                              float impulse_temperature)
{
    ...
    ImpulseDensity(*density, impulse_position, hotspot, splat_radius,
                   impulse_density);
...
void GridFluidSolver::Solve(GraphicsVolume* density, float delta_time)
{
    ...
    AdvectDensity(density, delta_time);
...
void GridFluidSolver::AdvectDensity(GraphicsVolume* density,
                                    float delta_time)
{
    ...
    if (graphics_lib_ == GRAPHICS_LIB_CUDA) {
        ...
        AdvectImpl(*density, delta_time, density_dissipation);
    }
    density->Swap(*general1a_);
}
...
void GridFluidSolver::ApplyBuoyancy(const GraphicsVolume& density,
                                    float delta_time)
{
...
void GridFluidSolver::ImpulseDensity(const GraphicsVolume& density,
                                     const glm::vec3& position,
                                     ...
                                     float splat_radius, float value)
{
    if (graphics_lib_ == GRAPHICS_LIB_CUDA) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER,
                          density.gl_volume()->frame_buffer());
        glEnable(GL_BLEND);
        glDrawArraysInstanced(GL_TRIANGLE_STRIP, 0, 4,
                              density.gl_volume()->depth());
        ResetState();
    }
}

void GridFluidSolver::ReviseDensity(const GraphicsVolume& density)
{
    ...
    //            density.cuda_volume(), pos, grid_size_.x * 0.5f, 0.1f);
    //}
}
