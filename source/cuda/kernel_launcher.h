...
extern void EmitParticles(const FlipParticles& particles, float3 center_point, float3 hotspot, float radius, float density, float temperature, uint random_seed, uint3 volume_size, BlockArrangement* ba);
...
extern void SortParticles(FlipParticles particles, int* num_active_particles, uint16_t* aux, uint3 volume_size, BlockArrangement* ba, AuxBufferManager* bm);
