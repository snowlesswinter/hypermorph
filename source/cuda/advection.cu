...
__global__ void AdvectFieldBfeccKernel(float time_step, float dissipation)
{
    ...
    float3 vel;
    vel.x = tex3D(tex_vx, coord.x, coord.y, coord.z);
    vel.y = tex3D(tex_vy, coord.x, coord.y, coord.z);
    vel.z = tex3D(tex_vz, coord.x, coord.y, coord.z);
    float3 back_traced = coord - time_step * vel;
...
__global__ void AdvectFieldSemiLagrangianKernel(float time_step, float dissipation)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int z = blockIdx.z * blockDim.z + threadIdx.z;
...
__global__ void BfeccRemoveErrorKernel(float time_step)
{
    ...
    vel.y = tex3D(tex_vy, coord.x, coord.y, coord.z);
    vel.z = tex3D(tex_vz, coord.x, coord.y, coord.z);
    float3 forward_trace = coord + time_step * vel;
                       ...
                       uint3 volume_size, BlockArrangement* ba)
{
    ...
    for (int i = 0; i < num_of_fields; i++) {
        ...
        AdvectFieldSemiLagrangianKernel<<<grid, block>>>(time_step, 0.0f);
                            ...
                            uint3 volume_size, BlockArrangement* ba)
{
    ...
    for (int i = 0; i < num_of_fields; i++) {
        ...
        AdvectFieldMacCormackKernel<<<grid, block>>>(time_step, dissipation);
    }
}
